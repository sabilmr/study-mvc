...
public class AkademikController {
    ...
    public ModelAndView detail(@PathVariable int id){
        ...
        Optional<FakultasModel> model = fakultas.stream().filter(f -> f.getId() == id).findFirst();
        if(model.isPresent()){
    ...
    public ModelAndView jurusan(@PathVariable int fakultasId,
                                @PathVariable int jurusanId){
        ModelAndView view = new ModelAndView("pages/akademik/detail/detail-jurusan");

        Optional<FakultasModel> fakultasModel = fakultas.stream().filter(f -> f.getId() == fakultasId).findFirst();
        if(fakultasModel.isPresent()){
            JurusanModel jurusanModel = fakultasModel.get().getJurusan().stream().filter(j -> j.getId() == jurusanId).findFirst().get();
